# ==========================================================================
#  NolimitzBots - My Performance (step 2: analysis)
#  Cost per hour, behaviour after losing runs, and breakdowns by market,
#  contract, stake size, hour of day and day of week.
#
#  Only touches the My Performance files. The data layer is unchanged.
#
#      powershell -ExecutionPolicy Bypass -File .\install-my-performance-2.ps1
#
...
Set-Location $root
if (-not (Test-Path 'src/components/shared/nlb/history-store.ts')) { Fail 'Step 1 not installed. Run the My Performance installer first.' }
Info "Repo: $root"
...
.my-performance {
    ...
    &__row.five {
        grid-template-columns: 1.4fr 0.8fr 1.2fr 1fr 1fr;
    }
...
git add -A
git commit -m "My Performance: cost per hour, loss-streak behaviour, and breakdowns"
...
Write-Host ''
Write-Host 'Done. Hard refresh My Performance once Vercel is green.' -ForegroundColor Yellow
