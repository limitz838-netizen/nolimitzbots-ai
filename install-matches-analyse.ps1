# ==========================================================================
#  NolimitzBots - Matches Pro: Analyse button
#  Press Analyse to get a digit with a countdown window to act in. The window
#  scores itself against the ticks that arrive inside it, and the button keeps
#  a running tally of its own hit rate against the 10% random baseline.
#
#      powershell -ExecutionPolicy Bypass -File .\install-matches-analyse.ps1
#
...
Set-Location $root
if (-not (Test-Path 'src/components/shared/nlb/matches-engine.ts')) { Fail 'Matches Pro Phase 2 not installed.' }
Info "Repo: $root"
...
Write-File 'src/components/shared/nlb/analyse-store.ts' $analyseSrc
...
git add -A
git commit -m "Matches Pro: Analyse button with self-scoring countdown window"
...
Write-Host ''
Write-Host 'Done. Hard refresh Matches Pro once Vercel is green.' -ForegroundColor Yellow
