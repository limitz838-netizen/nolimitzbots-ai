# ==========================================================================
#  NolimitzBots - Matches Pro, Phase 3
#  DIGITMATCH execution with a risk guard. DEMO ACCOUNTS ONLY.
#
#  Hard locks enforced in risk-guard.evaluate(), not in the UI:
#    * a real login id is refused at the execution layer
#    * a market cannot auto-trade below 500 graded predictions
#    * only STRONG or MEDIUM signals are ever sent
#
#      powershell -ExecutionPolicy Bypass -File .\install-matches-pro-phase3.ps1
#
...
Set-Location $root
if (-not (Test-Path 'src/components/shared/nlb/matches-engine.ts')) { Fail 'Phase 2 not installed. Run the Phase 2 installer first.' }
if (-not (Test-Path 'src/components/shared/nlb/settlement.ts')) { Fail 'settlement.ts missing - unexpected repo state.' }
Info "Repo: $root"
...
const MatchesPro = () => {
    ...
    const setLimit = (key, value) => {
        const next = { ...limits_ref.current, [key]: Number(value) };
        setLimits(next);
        saveLimits(next);
    };
        ...
        <div className='matches-pro'>
            <div className='matches-pro__panel'>
                ...
                <div className='matches-pro__note'>
                    ...
                    hidden here. Auto trading unlocks per market at {MIN_EVIDENCE} graded predictions, and only STRONG
                    or MEDIUM signals are ever sent. Every trade still faces a negative expected value at a{' '}
...
Info ''
Info '[1/3] Writing Phase 3 files'
...
git add -A
git commit -m "Matches Pro Phase 3: demo-only DIGITMATCH execution with risk guard"
...
Write-Host ''
Write-Host 'Done. Switch to your DEMO account before opening Matches Pro.' -ForegroundColor Yellow
